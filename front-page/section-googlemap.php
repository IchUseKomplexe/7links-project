<div class="front_page_section front_page_section_googlemap<?php
			$smart_casa_scheme = smart_casa_get_theme_option('front_page_googlemap_scheme');
			if (!smart_casa_is_inherit($smart_casa_scheme)) echo ' scheme_'.esc_attr($smart_casa_scheme);
			echo ' front_page_section_paddings_'.esc_attr(smart_casa_get_theme_option('front_page_googlemap_paddings'));
		?>"<?php
		$smart_casa_css = '';
		$smart_casa_bg_image = smart_casa_get_theme_option('front_page_googlemap_bg_image');
...
?>><?php
	// Add anchor
	$smart_casa_anchor_icon = smart_casa_get_theme_option('front_page_googlemap_anchor_icon');	
	$smart_casa_anchor_text = smart_casa_get_theme_option('front_page_googlemap_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_googlemap_inner<?php
			if (smart_casa_get_theme_option('front_page_googlemap_fullheight'))
				echo ' smart_casa-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$smart_casa_css = '';
			$smart_casa_bg_mask = smart_casa_get_theme_option('front_page_googlemap_bg_mask');
			$smart_casa_bg_color = smart_casa_get_theme_option('front_page_googlemap_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_googlemap_content_wrap<?php
			$smart_casa_layout = smart_casa_get_theme_option('front_page_googlemap_layout');
			if ($smart_casa_layout != 'fullwidth')
				echo ' content_wrap';
		?>">
			<?php
			// Content wrap with title and description
			$smart_casa_caption = smart_casa_get_theme_option('front_page_googlemap_caption');
			$smart_casa_description = smart_casa_get_theme_option('front_page_googlemap_description');
			if (!empty($smart_casa_caption) || !empty($smart_casa_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				if ($smart_casa_layout == 'fullwidth') {
					...
					if (!empty($smart_casa_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
						?><h2 class="front_page_section_caption front_page_section_googlemap_caption front_page_block_<?php echo !empty($smart_casa_caption) ? 'filled' : 'empty'; ?>"><?php
					...
					if (!empty($smart_casa_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
						?><div class="front_page_section_description front_page_section_googlemap_description front_page_block_<?php echo !empty($smart_casa_description) ? 'filled' : 'empty'; ?>"><?php
			...
			// Content (text)
			$smart_casa_content = smart_casa_get_theme_option('front_page_googlemap_content');
			if (!empty($smart_casa_content) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				if ($smart_casa_layout == 'columns') {
					?><div class="front_page_section_columns front_page_section_googlemap_columns columns_wrap">
						<div class="column-1_3">
					<?php
				} else if ($smart_casa_layout == 'fullwidth') {
					?><div class="content_wrap"><?php
				}
	
				?><div class="front_page_section_content front_page_section_googlemap_content front_page_block_<?php echo !empty($smart_casa_content) ? 'filled' : 'empty'; ?>"><?php
			...
			?><div class="front_page_section_output front_page_section_googlemap_output"><?php 
				if (is_active_sidebar('front_page_googlemap_widgets')) {
					...
					if (!smart_casa_exists_trx_addons())
						...
						smart_casa_customizer_need_widgets_message('front_page_googlemap_caption', 'ThemeREX Addons - Google map');
				}
			?></div><?php
