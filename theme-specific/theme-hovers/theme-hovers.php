...
if (!function_exists('smart_casa_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'smart_casa_hovers_theme_setup3', 3 );
	function smart_casa_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		smart_casa_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'smart-casa'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'smart-casa') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'smart-casa'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'smart-casa'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'smart-casa') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'icon'	=> esc_html__('Icon',	'smart-casa'),
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('smart_casa_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'smart_casa_hovers_theme_setup9', 9 );
	function smart_casa_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',			'smart_casa_hovers_frontend_scripts', 1010 );
		add_filter( 'smart_casa_filter_localize_script','smart_casa_hovers_localize_script' );
		add_filter( 'smart_casa_filter_merge_scripts',	'smart_casa_hovers_merge_scripts' );
		add_filter( 'smart_casa_filter_merge_styles',	'smart_casa_hovers_merge_styles' );
		add_filter( 'smart_casa_filter_merge_styles_responsive', 'smart_casa_hovers_merge_styles_responsive' );
		add_filter( 'smart_casa_filter_get_css', 		'smart_casa_hovers_get_css', 10, 2 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'smart_casa_hovers_frontend_scripts' ) ) {
	
	function smart_casa_hovers_frontend_scripts() {
		if ( smart_casa_is_on(smart_casa_get_theme_option('debug_mode')) && smart_casa_get_file_dir('theme-specific/theme-hovers/theme-hovers.js')!='' )
			wp_enqueue_script( 'smart-casa-hovers', smart_casa_get_file_url('theme-specific/theme-hovers/theme-hovers.js'), array('jquery'), null, true );
	}
}

// Merge hover effects into single css
if (!function_exists('smart_casa_hovers_merge_styles')) {
	
	function smart_casa_hovers_merge_styles($list) {
...
if (!function_exists('smart_casa_hovers_merge_styles_responsive')) {
	
	function smart_casa_hovers_merge_styles_responsive($list) {
...
if (!function_exists('smart_casa_hovers_localize_script')) {
	
	function smart_casa_hovers_localize_script($arr) {
		$arr['button_hover'] = smart_casa_get_theme_option('button_hover');
		return $arr;
	}
}

// Merge hover effects into single js
if (!function_exists('smart_casa_hovers_merge_scripts')) {
	
	function smart_casa_hovers_merge_scripts($list) {
...
if ( !function_exists('smart_casa_hovers_add_icons') ) {
	function smart_casa_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat'])) {
					smart_casa_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? ($product->is_type( 'variable' ) ? esc_html__('Select options', 'smart-casa') : esc_html__('Buy now', 'smart-casa')) : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'smart-casa');
						else
							esc_html_e('Details', 'smart-casa');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php echo !empty($post_link) ? esc_url($post_link) : esc_attr($no_link); ?>" aria-hidden="true" class="icon-plus-1"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'smart_casa_hovers_get_css' ) ) {
	
	function smart_casa_hovers_get_css($css, $args) {
