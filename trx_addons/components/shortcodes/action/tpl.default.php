...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	...
	foreach ($args['actions'] as $item) {
		...
		?><div class="sc_action_item sc_action_item_<?php
			...
			if (!empty($item['height']) && (int)$item['height'] > 0 && (int)$args['full_height'] == 0) echo ' sc_action_fixed_height';
			...
			if (!empty($item['bg_image']) || !empty($item['bg_color']) || (!empty($item['height']) && (int)$args['full_height'] == 0)) 
				echo ' style="'
							...
							. (!empty($item['height']) && (int)$item['height'] > 0 && (int)$args['full_height'] == 0 ? 'height:'.esc_attr($item['height']).';overflow:hidden;' : '')
							. '"'; 
		?>>
			<?php
			...
			if (!empty($item['image'])) {
				...
				if (!empty($item['image'])) {
					...
					?><div class="sc_action_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Image', 'smart-casa'); ?>" <?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
				...
				if (!empty($icon)) {
					...
					?><div class="sc_action_item_icon sc_action_item_icon_type_<?php echo esc_attr($item['icon_type']); ?> <?php echo esc_attr($icon); ?>"
						...
						if (!empty($img)) {
							...
							?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'smart-casa'); ?>" <?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						} else {
			...
			if (!empty($item['link']) && !empty($item['link_text'])) {
				?><a href="<?php echo esc_url($item['link']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_action_item_link sc_button', 'sc_action', $args, $item)); ?>"><?php
