/* global smart_casa_color_schemes, smart_casa_dependencies, Color */
...
( function( api ) {
	...
	for (var i in smart_casa_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'smart_casa-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'smart_casa-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'smart_casa-vars' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+smart_casa_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(smart_casa_customizer_vars['msg_reset_confirm'])) {
		...
		if (smart_casa_customizer_vars && smart_casa_customizer_vars['theme_name_suffix'])
			jQuery('#customize-info .site-title').append(smart_casa_customizer_vars['theme_name_suffix']);

		// Blur the "load fonts" fields - regenerate options lists in the font-family controls
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', smart_casa_customizer_update_load_fonts);		

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', smart_casa_customizer_actions);

		// Check dependencies in the each section on ready
		jQuery('#customize-theme-controls .control-section').each(function () {
			smart_casa_customizer_check_dependencies(jQuery(this));
		});
		...
		jQuery('#customize-theme-controls .control-section > .accordion-section-title').on('click', function() {
			...
			if (id!='') {
				...
				if (section.length > 0) smart_casa_customizer_check_dependencies(section);
			}
		});
	...
	api.bind('change', function(obj) {
		...
		smart_casa_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		smart_casa_customizer_refresh_preview(obj);
	});
	...
	api.bind('refresh_schemes', function() {
		...
		for (var i in cssTemplate) {
			if ((''+i).indexOf('scheme_') !== 0) continue;
			i = i.replace('scheme_', '');
			if (typeof smart_casa_color_schemes[i] === 'undefined') {
				delete cssTemplate['scheme_'+i];
				jQuery('#tmpl-smart_casa-color-scheme-'+i).remove();
			} else if (tpl_idx == '') {
				tpl_idx = i;
				tpl_content = jQuery( '#tmpl-smart_casa-color-scheme-' + i ).html().trim();
			}
		}
		// Add new templates
		...
		for (var i in smart_casa_color_schemes) {
			if (typeof cssTemplate['scheme_'+i] === 'undefined') {
				jQuery( '#tmpl-smart_casa-color-scheme-' + tpl_idx )
					.clone()
					.attr('id', 'tmpl-smart_casa-color-scheme-' + i)
					.html(tpl_content.replace(regex, '.scheme_' + i))
					.insertAfter(jQuery( '#tmpl-smart_casa-color-scheme-' + tpl_idx ));
				cssTemplate['scheme_'+i] = wp.template( 'smart_casa-color-scheme-'+i );
			}
		}
	});
	...
	for (var action in smart_casa_customizer_vars['actions']['expand']) {
		if (action == 'length') continue;
		if (smart_casa_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel( action, function( panel ) {
				panel.expanded.bind( function( isExpanded ) {
					if ( isExpanded ) {
						var data = smart_casa_customizer_vars['actions']['expand'][panel.id];
			...
			api.section( action, function( section ) {
				section.expanded.bind( function( isExpanded ) {
					if ( isExpanded ) {
						var data = smart_casa_customizer_vars['actions']['expand'][section.id];
						if (typeof data['url'] !== 'undefined' && data['url'] != '')
							api.previewer.previewUrl.set( data['url'] );
						if (typeof data['callback'] !== 'undefined' && data['callback'] != '' && typeof window[data['callback']] === 'function')
							window[data['callback']]();
	...
	function smart_casa_customizer_get_field_value(fld) {
	...
	function smart_casa_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			...
			if (depend) {
				...
				for (i in depend) {
					...
					if (fld.length > 0) {
						val = smart_casa_customizer_get_field_value(fld);
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					ctrl.show().removeClass('smart_casa_options_no_use');
				} else {
					ctrl.hide().addClass('smart_casa_options_no_use');
				}
			}
			...
			if (false && id == 'color_scheme') {
				...
				if (fld.length > 0) {
					val = smart_casa_customizer_get_field_value(fld);
					var num = 0;
					for (i in smart_casa_color_schemes) {
						num++;
						if (i == val) break;
					}
					cont.find('.customize-control').each(function() {
	...
	function smart_casa_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			// Any color in the scheme_storage is changed
			} else if (Object.values(smart_casa_sorted_schemes).indexOf(id) >= 0) {
				forceUpdateCSS = true;
			...
			// If control from the theme vars
				...
				if (var_name !== undefined) {
					...
					smart_casa_customizer_update_theme_vars(jQuery('[data-customize-setting-link="'+id+'"]').data('var_name'), val);
					processed = true;
				}

			// Any theme fonts parameter is changed
			} else {
				for (opt in smart_casa_theme_fonts) {
					for (rule in smart_casa_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value to the fonts table
							smart_casa_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed || forceUpdateCSS) smart_casa_customizer_update_css();
		}
	...
	// Actions buttons
	function smart_casa_customizer_actions(e) {
	...
	function smart_casa_customizer_update_theme_vars(opt, value) {
		smart_casa_theme_vars[opt] = parseFloat(value);
	}
	
	// Store new value in the theme fonts
	function smart_casa_customizer_update_theme_fonts(opt, rule, value) {
		smart_casa_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function smart_casa_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i <= smart_casa_customizer_vars['max_load_fonts']; i++) {
	...
	function smart_casa_customizer_update_css() {
		...
		var vars = smart_casa_customizer_add_theme_vars(smart_casa_theme_vars);

		// Make styles and add its to the css
		css += smart_casa_customizer_prepare_html_value(cssTemplate['theme_vars']( vars ));

		// Add theme specific fonts rules
		var fonts = smart_casa_customizer_add_theme_fonts(smart_casa_theme_fonts);

		// Make styles and add its to the css
		css += smart_casa_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in smart_casa_sorted_schemes) {
			if (param == 'length') continue;
			scheme = api(smart_casa_sorted_schemes[param])();
			if (scheme && scheme !== 'inherit') sorted[scheme] = 1;
		}
		for (scheme in smart_casa_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in smart_casa_color_schemes[scheme].colors) {
				colors[i] = smart_casa_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = smart_casa_customizer_add_theme_colors(colors);
            ...
            var tmpl = jQuery( '#tmpl-smart_casa-color-scheme-' + scheme ).html().trim();
            for (i in colors) {
	...
	function smart_casa_customizer_add_theme_colors(colors) {
		if (smart_casa_additional_colors) {
			var clr = '', v = '';
			for (var k in smart_casa_additional_colors) {
				v = smart_casa_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' 
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + smart_casa_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' 
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + smart_casa_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' 
									&& fonts[tag]['margin-bottom'] != '' 
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + smart_casa_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	function smart_casa_customizer_add_theme_vars(vars) {
		...
		for (var i in vars) {
			if (i == 'rad') {
				if (vars['rad'] == '') vars['rad'] = 0;
				rez['rad'] = smart_casa_customizer_prepare_css_value(vars['rad']);
				rez['rad50'] = vars[i]>0 ? '50%' : 0;
				rez['rad4']  = vars[i]>0 ? '4px' : 0;
			} else if (['page', 'sidebar', 'gap'].indexOf(i) >= 0) {
				vars['page'] = parseInt(vars['page'], 10);
				if (isNaN(vars['page']) || vars['page'] == 0) vars[i] = 1170;
				vars['sidebar'] = parseInt(vars['sidebar'], 10);
				vars['gap'] = parseInt(vars['gap'], 10);
				rez['page'] = smart_casa_customizer_prepare_css_value(vars['page']);
				rez['sidebar'] = smart_casa_customizer_prepare_css_value(vars['sidebar']);
				rez['gap'] = smart_casa_customizer_prepare_css_value(vars['gap']);
				rez['content'] = smart_casa_customizer_prepare_css_value(vars['page'] - vars['sidebar'] - vars['gap']);
				rez['sidebar_gap'] = smart_casa_customizer_prepare_css_value(vars['sidebar'] + vars['gap']);
				rez['sidebar_prc'] = vars['sidebar'] / vars['page'];
				rez['gap_prc'] = vars['gap'] / vars['page'];
				rez['page_boxed'] = smart_casa_customizer_prepare_css_value(vars['page'] + 2 * 60);
				rez['page_1_1'] = smart_casa_customizer_prepare_css_value(vars['page']);
				rez['page_1_2'] = smart_casa_customizer_prepare_css_value(vars['page'] / 2);
				rez['page_1_3'] = smart_casa_customizer_prepare_css_value(vars['page'] / 3);
				rez['page_2_3'] = smart_casa_customizer_prepare_css_value(vars['page'] / 3 * 2);
				rez['page_1_4'] = smart_casa_customizer_prepare_css_value(vars['page'] / 4);
				rez['page_3_4'] = smart_casa_customizer_prepare_css_value(vars['page'] / 4 * 3);
				var margins = [10,20,30,40,60];
				for (var m=0; m<margins.length; m++) {
					rez['page'+margins[m]] = smart_casa_customizer_prepare_css_value(vars['page'] + margins[m]);
					rez['content'+margins[m]] = smart_casa_customizer_prepare_css_value(vars['page'] - vars['gap'] - vars['sidebar'] + margins[m]);
				}
			}
		}
		return rez;
	...
	// Add ed to css value
	function smart_casa_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = (''+val).substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function smart_casa_customizer_prepare_html_value(val) {
